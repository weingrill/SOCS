...
class GaiaCluster(object):
    ...
    def __init__(self, clustername, ra=None, dec=None, diam=None, dist=None, pmra=None, pmdec=None, pmradius=5.0, depth=0.5):
        '''
        Constructor
        '''
    ...
    def gaiaquery(self):
        ...
        min_dist = self.dist * 0.5
        max_dist = self.dist * 1.5
        if self.depth >= 10.0 and self.depth < self.dist:
            min_dist = self.dist - self.depth
            max_dist = self.dist + self.depth
        elif 0.0 < self.depth < 1.0:
            min_dist = self.dist * (1.0 - self.depth)
            max_dist = self.dist * (1.0 + self.depth)
...
if __name__ == '__main__':
    ...
    # c = GaiaCluster('NGC 2682', ra=132.85, dec=11.82, pmra=-10.98, pmdec=-2.99, diam=120.0, pmradius=2.0,
    #                 dist=879.4)  # M 67
